// Data structure that holds a single Notification's data.

enum NotificationType { Tweet, Email, TextMessage, PhoneCall, VoiceMail }

class Notification {
   
  int timestamp;
  NotificationType type; //Tweet, Email, TextMessage, PhoneCall, VoiceMail
  String message; //NOT used by PhoneCall
  String sender;
  int priority;
  int contentSummary; //NOT used by Tweet or PhoneCall
  ...
  public String toString() {
      String output = getType().toString() + "[" + getTimestamp() + "]: ";
      output += "(" + getSender() + ")";
      output += " " + getMessage();
      return output;
    }
